...
// ------------------------------------------------------------------------- //
	...
	function import_spiders_list()
	{
		...
		foreach ($spiders as $id => $spider) {
			...
			$formobjects[$id]->setDescription('<a href="'.$_SERVER['PHP_SELF'].'?op=edit&id='.$id.'">'._EDIT.'</a>&nbsp;|&nbsp;<a href="'.$_SERVER['PHP_SELF'].'?op=delete&id='.$id.'">'._DELETE.'</a>');
	...
	function import_spiders_edit($id)
	{
		...
		$required = array('robot-safeuseragent', 'robot-useragent', 'robot-exclusion-useragent', 'robot-name', 'robot-id');

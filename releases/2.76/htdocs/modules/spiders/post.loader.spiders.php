...
// ------------------------------------------------------------------------- //
	...
	foreach($spiders as $spider) {
		...
		if (((intval($match/count($uagereg)*100)+intval($match/count($usersafeagent)*100))/2)>intval($GLOBALS['spidersModuleConfig']['match_percentile'])) {
			...
			$statistic->setVar('uri', XOOPS_PROT.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
			...
			$sql = 'DELETE FROM '.$GLOBALS['xoopsDB']->prefix('spiders_statistics').' WHERE `when` < '.time()-($GLOBALS['spidersModuleConfig']['weeks_stats']*(7*24*60*60));
			...
			$online_handler->write($suser->getVar('uid'), $GLOBALS['xoopsUser']->getVar('uname'), time(), $GLOBALS['spidersModule']->getVar('mid'), (string)$_SERVER["REMOTE_ADDR"]);

...
class SpidersSpidersHandler extends XoopsPersistableObjectHandler
{
	...
	function safeAgent($useragent) {
		$sub_char = array('(', '|', '{', '[','at', 'xxx');
		foreach($sub_char as $sc) {
			if (strpos($useragent, $sc))
				$useragent = trim(substr($useragent, 0, strpos($useragent, $sc)));
		}
		...
		if (strtolower(substr($useragent, 0, 7))=='mozilla')
			$useragent = md5(rand(0,30000));

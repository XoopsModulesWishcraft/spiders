...
class CURLSpidersExchange {
	...
	function sendSpider($spider) {
		...
		case "PHPCURL":
			$GLOBALS['sJSON'] = new Services_JSON();
			curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, 'spider='.$GLOBALS['sJSON']->encode(array(      "username"	=> 	$this->curl_xoops_username, "password"	=> 	$this->curl_xoops_password, "spider"	=> 	$spider	)));
	...
	function sendStatistic($statistic) {
		...
		case "PHPCURL":
			$GLOBALS['sJSON'] = new Services_JSON();
			curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, 'spiderstat='.$GLOBALS['sJSON']->encode(array(      "username"	=> 	$this->curl_xoops_username, "password"	=> 	$this->curl_xoops_password, "statistic"	=> 	$statistic	)));
			$data = curl_exec($this->curl_client);
			curl_close($this->curl_client);
			$result = spiders_obj2array($GLOBALS['sJSON']->decode($data));		
			break;
	...
	function getSpiders() {
		...
		case "PHPCURL":
			$GLOBALS['sJSON'] = new Services_JSON();
			curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, 'spiders='.$GLOBALS['sJSON']->encode(array( "username" => $this->curl_xoops_username, "password"	=> 	$this->curl_xoops_password )));
			$data = curl_exec($this->curl_client);
			curl_close($this->curl_client);
			$result = spiders_obj2array($GLOBALS['sJSON']->decode($data));		
			break;
	...
	function getSEOLinks() {
		...
		case "PHPCURL":
			$GLOBALS['sJSON'] = new Services_JSON();
			curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, 'seolinks='.$GLOBALS['sJSON']->encode(array( "username" => $this->curl_xoops_username, "password"	=> 	$this->curl_xoops_password )));
			$data = curl_exec($this->curl_client);
			curl_close($this->curl_client);	
			$result = spiders_obj2array($GLOBALS['sJSON']->decode($data));		

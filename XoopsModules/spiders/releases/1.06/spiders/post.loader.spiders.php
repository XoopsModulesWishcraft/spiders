...
// ------------------------------------------------------------------------- //
	...
	if ($installed!=0)
	{
		...
		if ($module->getVar('isactive')==true)
		{
			...
			$sa = $spider_handler->safeAgent($_SERVER['HTTP_USER_AGENT']);
			$part = explode('/', $sa);
			foreach($part as $value) 
				if (!is_numeric((substr($value,0,1)))&&(substr($value,0,1))!='x')
					if (!empty($value)) {
						$criteria->add(new Criteria('`robot-safeuseragent`', '%'.$value.'%', 'LIKE'));
						$uagereg[] = $value;
					}
			
			$spiders = $spider_handler->getObjects($criteria);
			foreach($spiders as $spider) {
				$suser = $suser_handler->get($spider->getVar('id'));
				$robot = &$member_handler->getUser( $suser->getVar('uid') );
				foreach($uagereg as $ireg)
				if(in_array($ireg,explode('/',$spider->getVar('robot-safeuseragent')))) {		
					/**
					 * User Sessions
					 */
					$xoopsUser = '';
